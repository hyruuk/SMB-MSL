...
| **Session type** | `training` or `test` |
| **Session number** | Integer session index |
| **Blocks / Reps** | Number of blocks (training) or repetitions per sequence (test) |

### Session types

**Training** — 6 trained sequences, blocks of 24 trials (4 reps per sequence). Each trial has two executions: first with digits visible, then from memory. An adaptive reward system awards 0, 1, or 3 points based on accuracy and speed.

**Behavioral test** — All 12 sequences (6 trained + 6 untrained) intermixed. Digits remain visible for both executions. No points system.
...
| `session_type` | `training` or `test` |
| `session_number` | Session index |
| `block_number` | Block number (1 for test sessions) |
| `trial_number` | Global trial counter |
| `sequence_id` | Sequence identifier (1–12) |
| `sequence_digits` | Target sequence (semicolon-separated) |
| `execution_number` | 1 (guided) or 2 (memory/repeat) |
...
| `points_awarded` | Points for this execution (training only) |
...
- Timing parameters (timeouts, inter-trial intervals)
...
└── berlot2020_task/
    ...
    ├── sequences.py       # 12 sequence definitions and group logic
    ├── display.py         # Visual stimuli and feedback screens
    ├── data_logging.py    # TSV file writer
    ├── task_training.py   # Training session logic
    └── task_test.py       # Behavioral test session logic
```
